...
public class UserProvider {
    ...
    public PostLoginRes logIn(PostLoginReq postLoginReq) throws BaseException{
        ...
        if(checkStatus(postLoginReq.getEmail()).equals("deleted")){
            throw new BaseException(DELETED_USER); // 탈퇴한 회원
        }

        User user = userDao.getPwd(postLoginReq);
        String password;
        try {
            password = new AES128(Secret.USER_INFO_PASSWORD_KEY).decrypt(user.getPw());
